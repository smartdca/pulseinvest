...
async function renderExpandedReport(ticker, containerId) {
  ...
  try {
    ...
    // 訊號紀錄卡左上角 logo(比照首頁推薦卡片,用同一個 createLogoImg)。
    // logo.js 這輪也補上了 turnstile token,新資產的 logo 才不會因為 403 掉到文字後備。
    const logoBox = document.getElementById(`er-record-logo-${ticker}`);
    if (logoBox && typeof createLogoImg === 'function') {
      logoBox.innerHTML = '';
      logoBox.appendChild(createLogoImg(ticker, 34));
    }
...
function erComputeStats(series) {
  ...
  // 今天分數的歷史百分位排名:X% = 歷史上有多少比例的週分數比今天低(= 那些時候比現在貴)
  ...
  // 客觀極值週(視窗內最高分/最低分——注意是「這10年內」,不是史上,文案已對應調整)
  ...
  // 訊號紀錄卡用:全部是這條週序列自己的統計,不牽涉即時分數,所以永遠跟展開報表其他數字一致
  // (不會出現「跟主卡即時分數對不上」的問題——那個問題只發生在拿即時分數去比較的地方,這裡沒有)。
  const avgScore = allScores.reduce((a, b) => a + b, 0) / n;
  const triggerPct = triggered.length / n * 100;
  ...
  return {
    ...
    highWeek, lowWeek, avgScore, triggerPct,
  };
}
...
// ── 便宜度(取代原本的「歷史定位」橫桿) ────────────────────────
// 這個值是「今天的DCA Score在這檔資產自己過去10年裡的百分位」——不是價格的百分位,
// 也跟公司好壞無關(爛公司大跌之後這個值一樣會很高)。所以措辭只描述相對貴賤,
// 不用「CP值/性價比」這類隱含品質判斷的詞。
//
// 改版前的三個問題(都已修正):
//   ① 圓點位置用了 100-pct,方向整個相反(便宜的資產指針跑到紅色端)
//   ② 文案「倒數X%」語意與實際相反(實際是「贏過X%」)
//   ③ 改成正向敘述後仍不直覺:數字高=便宜,但句子結尾是「比現在貴」,眼睛會讀反
// 現在改成不顯示數字,只用分段色塊的填滿程度 + 一個結論詞,方向永遠一致:
//   填得越滿 = 越綠 = 越便宜。
const ER_CHEAP_LEVELS = [
  { max: 20,  zh: '明顯偏貴',  en: 'Much pricier' },
  { max: 40,  zh: '偏貴',      en: 'Pricier' },
  { max: 60,  zh: '中間',      en: 'Middle' },
  { max: 80,  zh: '偏便宜',    en: 'Cheaper' },
  { max: 101, zh: '明顯便宜',  en: 'Much cheaper' },
];

function erCheapWord(pct, zh) {
  const lv = ER_CHEAP_LEVELS.find(l => pct < l.max) || ER_CHEAP_LEVELS[ER_CHEAP_LEVELS.length - 1];
  return zh ? lv.zh : lv.en;
}

// 紅→黃→綠,跟首頁 DCA Score 儀表同一套色階
function erCheapColor(pct) {
  const stops = [[0,[194,91,74]],[50,[224,176,136]],[75,[159,201,153]],[100,[74,157,110]]];
  ...
  return 'rgb(74,157,110)';
}

function erCheapSegments(pct) {
  const filled = Math.round(pct / 10);
  let out = '';
  for (let i = 0; i < 10; i++) {
    const bg = i < filled ? `background:${erCheapColor((i + 0.5) * 10)};` : '';
    out += `<div class="er-seg" style="${bg}"></div>`;
  }
  return `<div class="er-seg-row">${out}</div>`;
}
...
function erBuildHTML(ticker, data, s, zh) {
  ...
  // 年數一律顯示整數:跨度本來就依資產而異,「近10年」比「9.9年」好讀,
  // 而且年化報酬仍用未取整的 s.years 計算,顯示取整不影響任何數字正確性。
  const yrs = Math.round(s.years);
  ...
  <div class="er-summary">
    ...
    <div class="er-grid">
      <div class="er-box" style="position:relative;overflow:hidden;">
        <div class="er-lbl">${zh ? `近 ${yrs} 年成長` : `${yrs}-yr growth`}</div>
      ...
      <div class="er-box">
        <div class="er-lbl">${zh ? '現在有多便宜' : 'How cheap is it now'}</div>
        ${erCheapSegments(s.scoreRankPct)}
        <div style="text-align:center;margin-top:9px;">
          <div style="font-size:20px;font-weight:750;line-height:1.1;color:${erCheapColor(s.scoreRankPct)};">${erCheapWord(s.scoreRankPct, zh)}</div>
          <div style="font-size:10.5px;color:rgba(255,255,255,0.6);margin-top:3px;">${zh ? `對比這檔資產近 ${yrs} 年` : `vs. this asset's past ${yrs} years`}</div>
        </div>
      </div>
    </div>
    <div class="er-verdict">
      ${zh
        ? `回顧近 ${yrs} 年,系統總共標記出${s.episodeCount}次獨立的加碼機會${s.longestEpisode ? `,最長一次連續${s.longestEpisode.weeks}週` : ''}。若照這套訊號動態調整投入金額,任選2年區間比較,${s.winRate !== null ? s.winRate.toFixed(0) : '—'}%的區間報酬率都優於固定定額。`
        : `Over the past ${yrs} years the system flagged ${s.episodeCount} separate buy-more windows${s.longestEpisode ? `, the longest running ${s.longestEpisode.weeks} weeks` : ''}.`
      }
    </div>
  </div>
  ...
  <div class="er-card">
    ...
    <div class="er-legend">
      <span class="er-legend-item"><span class="er-swatch" style="background:var(--accent);"></span>${zh ? 'DCA Score(13週平均)' : 'Score (13wk avg)'}</span>
  ...
  <div class="er-card">
    <div class="er-record-head">
      <div class="er-record-logo" id="er-record-logo-${ticker}"></div>
      <div>
        <div class="er-card-title" style="margin-bottom:1px;">${zh ? '訊號紀錄 · 近 ' + yrs + ' 年' : 'Signal record · past ' + yrs + ' yrs'}</div>
        <div style="font-size:11px;color:var(--ink2);">${zh ? '以下數字都由資料本身決定,不是人工挑選' : 'Every figure below is decided by the data itself'}</div>
      </div>
    </div>

    <div class="er-extreme-row">
      <div class="er-extreme-box" style="background:var(--al);">
        <div class="er-extreme-lbl">${zh ? `近 ${yrs} 年最高分` : `${yrs}-yr high`}</div>
      ...
      <div class="er-extreme-box" style="background:var(--bg2);">
        <div class="er-extreme-lbl">${zh ? `近 ${yrs} 年最低分` : `${yrs}-yr low`}</div>
        <div class="er-extreme-val">${s.lowWeek.score}${zh ? '分' : ''}</div>
        <div class="er-extreme-sub">${s.lowWeek.week} · $${s.lowWeek.price}</div>
      </div>
    </div>

    <div class="er-avg-block">
      <div class="er-avg-top">
        <span class="er-avg-lbl">${zh ? `近 ${yrs} 年平均分` : `${yrs}-yr average score`}</span>
        <span class="er-avg-val">${s.avgScore.toFixed(1)}</span>
      </div>
      <div class="er-avg-track">
        <div class="er-avg-fill" style="width:${Math.max(2, Math.min(100, s.avgScore)).toFixed(1)}%;"></div>
        <div class="er-avg-mark" style="left:60%;" title="60"></div>
      </div>
      <div class="er-avg-ends">
        <span>0</span>
        <span>${zh ? '60 = 加碼門檻' : '60 = buy-more line'}</span>
        <span>100</span>
      </div>
    </div>
  </div>
...
function erDrawOverlayChart(series, svgId) {
  ...
  const N = series.length;
  // 2026-07-23:平滑從「4週尾隨」改成「13週置中」。
  //  · 窗口拉長到13週(一季):4週壓不住百分位算法本來就有的高頻擺動,線在0-100間衝,
  //    看不出跟價格的對應關係;13週把雜訊濾掉,但「價格跌→分數升」的形狀完整留著。
  //  · 改成置中:尾隨平均的重心會落在後面約 w/2 週,13週尾隨等於整條線右移約6週,
  //    分數的轉折比價格慢半拍——而這張圖的唯一任務就是看兩條線的對應關係。
  //    延遲是平滑動作造成的副作用,不是資料本身的性質(DCA Score 描述的是當下狀態,
  //    不是落後指標)。這張圖定位為「回顧」而非「模擬當時看到的畫面」,沒有因果性限制。
  //  ⚠️ 置中會用到後面幾週的資料,但這【只用於畫線】,不進入任何分數計算。
  //    後端 historical-score.js 的 expanding window 完全沒有更動,前視偏誤防護不受影響。
  //  · 已知取捨:最右端沒有「未來」可取,只剩半個窗口,線尾會略微不如中段平滑。
  const SMOOTH_WEEKS = 13;
  const H = Math.floor(SMOOTH_WEEKS / 2);
  const scores = series.map(d => d.score);
  const smooth = scores.map((_, i) => {
    const s = scores.slice(Math.max(0, i - H), Math.min(scores.length, i + H + 1));
