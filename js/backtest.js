...
function updateBTLabels(zh, r1, r2, ticker, benchmark) {
  ...
  if(ddLabel) ddLabel.textContent = isDesktop ? (zh ? '最大回撤 %' : 'Max %') : (zh ? '期間最深回撤' : 'Max Drawdown');
...
async function runBacktest() {
  ...
  try {
    ...
    if($('btMonthly')) $('btMonthly').textContent = Math.round(budget).toLocaleString('en-US');
    ...
    btCountUp($('btInvested'), r1.totalInvested, { duration: 900, decimals: 0, prefix: isDesktopStats ? '' : '$' });

    // 大數字(核心互動區,只剩累積金額,不帶%):比小數字晚停(1700ms)。
