...
const BT_WHATIF_CARDS = [
  ...
  {
    id: 'benchmark', icon: '🔀',
    label: { zh: '跟其它大盤比比看？', en: 'Compare against another index?' },
  ...
  {
    id: 'ticker', icon: '🔄',
    label: { zh: '改投其他資產？', en: 'Switch to a different asset instead?' },
  ...
  {
    id: 'more', icon: '👆',
    label: { zh: '想試試別的嗎？點擊進行更多試算。', en: 'Want to try another calculation? Click here.' },
    type: 'cta',
  },
...
function btRenderWhatIfChips(zh, ticker, benchmark, budget, years) {
  ...
  track.innerHTML = BT_WHATIF_CARDS.map((card, ci) => {
    ...
    // 「目前選中」高亮:年數比字串、對比/主角比代碼(不分大小寫)。金額卡是相對
    // 調整(±10%),沒有固定的「選中值」可比對,維持原本不高亮的邏輯。
    let curVal = null;
    if(card.field === 'btYears') curVal = String(years);
    else if(card.field === 'btBenchmark') curVal = (benchmark || '').toUpperCase();
    else if(card.field === 'btTicker') curVal = (ticker || '').toUpperCase();
    const optClass = card.style === 'pill-accent' ? 'bt-whatif-opt bt-whatif-opt-accent' : 'bt-whatif-opt';
    const btns = card.options.map((opt, oi) => {
      // 金額卡(btBudget)沒有「目前值」概念,兩顆維持原本一直都是實心的樣子
      // (已定案,不能被新的淡色/實心邏輯連累)。其餘三張才比對真正的目前值。
      const isActive = card.field === 'btBudget'
        ? true
        : (curVal != null && opt.value != null && String(opt.value).toUpperCase() === curVal);
