...
function updateBTLabels(zh, r1, r2, ticker, benchmark) {
  ...
  if(ddLabel) ddLabel.textContent = isDesktop ? (zh ? '最大回撤' : 'Max Drawdown') : (zh ? '期間最深回撤' : 'Max Drawdown');
...
async function runBacktest() {
  ...
  try {
    ...
    // 2026-08-25第六輪:情境行整條拿掉(NVDA/年份都已經在左欄看得到,每月投入
    // 改進五格統計常駐顯示),右上角只留純粹的大金額,不再帶%。
