...
**每週決定新資產之後、動手做圖或寫資料檔之前，先確認這支代號拿不拿得到財報。**
...
| 看到什麼 | 意思 | 怎麼辦 |
|---|---|---|
| `"partial": false`，有 `marketCap`／`peTTM`／`eps` | ✅ 可以做 | 照流程往下走 |
| `"partial": true`，`_debugBasic.errs` 是 `FMP 402` | ❌ 免費方案不涵蓋這支 | **換一支資產**，或接受五張數據卡顯示「更新中」 |
| `errs` 是 `FMP empty/invalid` | FMP 沒有這支的財報資料 | 同上 |
| `errs` 是 `FMP 429` | 當日 250 次額度用完 | 隔天再驗，不是這支的問題 |

**為什麼要先驗**：MU（2026-09）就是做完三張圖、寫完資料檔、全部上線之後，才發現財報抓不到。
五張數據卡（市值／本益比／EPS／年營收／殖利率）永遠停在「更新中」，
而且 `hasPfcf: false` 讓五因子少一項。頁面其餘功能都正常，但那五張卡補不回來。

**FMP 免費方案的已知限制**（2026-09 查證）：
- 每天 250 次請求，超過回 `429`
- 30 天內 500MB 流量
- 美股 5 年年報
- **部分代號不涵蓋,回 `402`** —— 官方定價頁沒有明講哪些代號,只能逐支實測。
  已知 TSLA / NFLX / NVDA / AAPL 可以,MU 不行。

驗證只要 10 秒,比做完一整輪才發現划算太多。
...
- [ ] 資產頁的五張數據卡有數字，不是「更新中」（若是，代表步驟 0 沒驗或驗過了仍決定上）
