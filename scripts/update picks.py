...
def fetch_yahoo(ticker, range_="1y", interval="1d", retries=2):
    ...
    # 補上更完整的瀏覽器標頭(不只 User-Agent),降低被判定為機器人流量的機率——
    # 2026-08-18 排查:GitHub Actions 的 IP 短時間內連續打上百次 Yahoo,被回 403/429
    # 整批擋掉,三個資產池全軍覆沒卻沒有任何錯誤訊號,原因見下方 run_weekly_picks()。
    headers = {
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if attempt < retries:
                # 退避時間拉長且遞增(2秒對付「被限流」幾乎無效,Yahoo的暫時封鎖通常要等更久):
                # 第一次重試等5秒,第二次等12秒
                time.sleep(5 if attempt == 0 else 12)
            else:
                raise e
