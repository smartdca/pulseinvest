...
def fetch_russell1000_growth():
    ...
    text = http_get(url, timeout=40).decode("utf-8-sig", "replace")
    ...
    for r in reader:
        ...
        if not t or (r.get("Asset Class") or "").strip() != "Equity":
            continue
        try:
            w = float((r.get("Weight (%)") or "0").replace(",", ""))
        except ValueError:
            w = 0.0
        rows.append((t, (r.get("Name") or "").strip(), w))
