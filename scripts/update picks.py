...
【候選池】(依「投資標的」分類,不是依工具;ETF 不是一個類別)
  ...
  growth       成長股     Russell 1000 成長股指數成分股,扣掉藍籌 100 檔後依權重前 100 名
                          (iShares IWF 持股檔)。非美國公司(ASML、ARM 等)不在 Russell
                          指數內,這個缺口是刻意接受的,不另外手動補。
...
def fetch_russell1000_growth():
    """Russell 1000 成長股指數成分股(iShares IWF 持股檔),依權重排序。回傳 [(ticker, name)]。"""
    # iShares 第一次造訪會先導到「投資人身分確認」的網頁(回傳 HTML 而不是 CSV)。
    # 做法:用同一個 cookie 容器先開一次產品頁(帶 siteEntryPassthrough=true 跳過確認頁),
    # 拿到網站給的 cookie 之後,再下載持股 CSV。
    import http.cookiejar
    product = "https://www.ishares.com/us/products/239706/ishares-russell-1000-growth-etf"
    csv_url = (product + "/1467271812596.ajax?fileType=csv&fileName=IWF_holdings"
               "&dataType=fund&siteEntryPassthrough=true")
    opener = urllib.request.build_opener(urllib.request.HTTPCookieProcessor(http.cookiejar.CookieJar()))
    base_headers = {"User-Agent": UA, "Accept-Language": "en-US,en;q=0.9"}
    try:
        opener.open(urllib.request.Request(product + "?siteEntryPassthrough=true",
                                           headers={**base_headers, "Accept": "text/html"}), timeout=40).read()
    except Exception as e:
        print(f"  iShares product page warm-up failed (continue anyway): {e!r}")
    text = opener.open(urllib.request.Request(csv_url, headers={**base_headers,
                       "Accept": "text/csv,text/plain,*/*", "Referer": product}), timeout=40).read().decode("utf-8-sig", "replace")
    lines = text.splitlines()
    # 持股檔前面有幾行基金資訊,真正的表頭是含 Ticker 與 Name 的那一行(欄名可能有引號)
    start = None
    for i, l in enumerate(lines[:60]):
        head = l.replace('"', "").replace("\ufeff", "").strip()
        if head.startswith("Ticker,") and "Name" in head:
            start = i
            break
    if start is None:
        # 抓不到表頭時,把回應開頭印出來,方便判斷是被擋(回傳網頁)還是格式改了
        snippet = text[:300].replace("\n", " | ")
        raise ValueError(f"IWF holdings header not found; response starts with: {snippet}")
    body = "\n".join(lines[start:])
    reader = csv.DictReader(io.StringIO(body))
    rows = []
    for r in reader:
        r = {(k or "").replace('"', "").strip(): (v or "").strip() for k, v in r.items()}
        t = r.get("Ticker", "")
        if not t or t == "-" or r.get("Asset Class", "") != "Equity":
            continue
        try:
            w = float(r.get("Weight (%)", "0").replace(",", ""))
        except ValueError:
            w = 0.0
        rows.append((t, r.get("Name", ""), w))
    rows.sort(key=lambda x: x[2], reverse=True)
    if len(rows) < 300:
        raise ValueError(f"Russell 1000 Growth only {len(rows)} holdings (expected ~390)")
...
def fetch_tw_top50():
    ...
    codes = []
    for code in re.findall(r"<td[^>]*>\s*(\d{4})\s*</td>", html):
        if code.startswith("00") or code in codes:
            continue
        codes.append(code)
        if len(codes) >= 50:
            break
    if len(codes) < 45:
        raise ValueError(f"TW market-cap ranking only {len(codes)} codes")
    return codes
...
def refresh_tickers(old):
    ...
    # 成長:Russell 1000 Growth 扣掉藍籌後前 100
    try:
        rg = dedupe_companies(fetch_russell1000_growth(),
                              exclude_tickers={SAME_COMPANY.get(t, t) for t in new["bluechip"]} | set(new["bluechip"]),
                              exclude_names=blue_names)
        new["growth"] = [t for t, _ in rg[:100]]
        print(f"  growth: {len(new['growth'])}")
    except Exception as e:
    ...
    with open(TICKERS_FILE, "w", encoding="utf-8") as f:
        json.dump({k: new[k] for k in ["updated", "index", "bluechip", "growth", "real", "crypto", "tw"]},
