...
<div class="wrap">
  <article>
    <div class="article-hero">
      <!-- lang-ok(b):備援網址,applyLang() 會用 window.dcaHref 依語言重寫 -->
...
function applyLang(lang){{
  ...
  /* 麵包屑要連回同語言的投資見解頁。判斷邏輯統一由 chrome.js 的 window.dcaHref
     提供(它讀 ZH_READY 清單);還沒載入完就維持英文網址,不會壞掉。 */
  try{{
    const bcEl = document.getElementById('breadcrumb');
    const bcHref = (typeof window.dcaHref === 'function') ? window.dcaHref('/insights.html') : '/insights.html';
    if(bcEl) bcEl.setAttribute('href', bcHref);
  }}catch(e){{}}
