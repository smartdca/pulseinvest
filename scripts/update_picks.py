...
def fetch_wikipedia_sp500():
    """Fetch S&P 500 tickers via Wikipedia's REST API (returns JSON, more reliable than HTML scraping)."""
    try:
        url = "https://en.wikipedia.org/w/api.php?action=parse&page=List_of_S%26P_500_companies&prop=wikitext&format=json"
        req = urllib.request.Request(url, headers={"User-Agent": "DCAcafe/1.0 (help@dcacafe.com)"})
        with urllib.request.urlopen(req, timeout=30) as r:
            data = json.loads(r.read())
        wikitext = data["parse"]["wikitext"]["*"]
        # Extract tickers from wikitext: rows look like "| AAPL || Apple Inc. || ..."
        # S&P 500 uses || as column separator
        tickers = re.findall(r'^\|\s*([A-Z]{1,5}(?:\.[A-Z])?)\s*\|\|', wikitext, re.MULTILINE)
        if not tickers:
        ...
        for t in tickers:
            if t not in seen:
                seen.add(t)
                unique.append(t)
        print(f"  S&P 500: found {len(unique)} tickers from wikitext")
        return unique
    except Exception as e:
        print(f"  Wikipedia S&P 500 API error: {e}")
        return []

def fetch_wikipedia_nasdaq100():
    """Fetch NASDAQ 100 tickers via Wikipedia's REST API."""
    try:
        url = "https://en.wikipedia.org/w/api.php?action=parse&page=Nasdaq-100&prop=wikitext&format=json"
        req = urllib.request.Request(url, headers={"User-Agent": "DCAcafe/1.0 (help@dcacafe.com)"})
        with urllib.request.urlopen(req, timeout=30) as r:
            data = json.loads(r.read())
        wikitext = data["parse"]["wikitext"]["*"]
        ...
        for t in tickers:
            if t not in seen:
                seen.add(t)
                unique.append(t)
        print(f"  NASDAQ 100: found {len(unique)} tickers from wikitext")
        return unique
    except Exception as e:
        print(f"  Wikipedia NASDAQ 100 API error: {e}")
        return []
