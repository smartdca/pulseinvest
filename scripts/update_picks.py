...
def find_unadjusted_split(closes):
    """找出 Yahoo 沒有還原的股票分割。回傳分割倍數(例如 7),沒有則回傳 None。

    實例:0052 富邦科技 2025-11 做了 1 拆 7,Yahoo 的價格(含還原價)都沒有還原,
    也沒有記錄分割事件,資料上就是單日從 248.5 掉到 35.57(相除 6.99)。
    這種假的暴跌會讓回撤、均線、歷史最大回撤全部失真,分數被墊高好幾年。

    判斷條件(三個都要符合,真正的崩跌幾乎不可能同時成立):
      · 單日價格變成原本的一半以下,或兩倍以上
      · 變動倍數非常接近整數(誤差 3% 以內),而且在 2~20 之間
      · 只用在非加密貨幣(呼叫端負責排除)"""
...
def screen_ticker(ticker, vix_closes):
    ...
    if not closes:
        return None
    # 有「沒還原的分割」的資產,分數不可信,直接排除在精選之外(加密貨幣不套用)
    if not ticker.upper().endswith("-USD"):
        split = find_unadjusted_split(closes)
        if split:
            print(f"  [{ticker}] skipped: unadjusted split detected (about 1-for-{split})")
            return None
