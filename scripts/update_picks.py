...
def fetch_wikipedia_sp500():
    """Fetch S&P 500 tickers by scraping Wikipedia HTML table."""
    try:
        url = "https://en.wikipedia.org/wiki/List_of_S%26P_500_companies"
        req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (compatible; DCAcafe/1.0)"})
        with urllib.request.urlopen(req, timeout=30) as r:
            html = r.read().decode("utf-8")
        # Table rows look like: <tr><td><a href="...">MMM</a>
        tickers = re.findall(r'<td><a[^>]+>([A-Z]{1,5}(?:\.[A-Z]{1,2})?)</a>', html)
        seen = set()
        unique = []
        for t in tickers:
            if t not in seen:
                seen.add(t)
                unique.append(t)
        print(f"  S&P 500: found {len(unique)} tickers from HTML")
        return unique
    except Exception as e:
...
def fetch_wikipedia_nasdaq100():
    """Fetch NASDAQ 100 tickers by scraping Wikipedia HTML table."""
    try:
        ...
        for t in tickers:
            if t not in seen:
                seen.add(t)
                unique.append(t)
        print(f"  NASDAQ 100: found {len(unique)} tickers from HTML")
        return unique
    except Exception as e:
